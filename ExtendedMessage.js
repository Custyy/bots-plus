    ...
    async yanıtla(content, options) {
        ...
        if (Array.isArray(apiMessage.data.content)) {
            return Promise.all(apiMessage.split().map(x => {
                ...
                return x
            }).map(this.yanıtla.bind(this)))
        }

        const { data, files } = await apiMessage.resolveFiles()
        return this.client.api.channels[this.channel.id].messages.post({ data, files }).then(dataa => this.client.actions.MessageCreate.handle(dataa).message)
    }
}
